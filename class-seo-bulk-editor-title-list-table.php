<?php 
	...
	class SEO_Bulk_Editor_Title_List_Table extends WP_List_Table {

		function __construct() {
			parent::__construct( array( 
				'singular' => 'seo_bulk_editor_title',
				'plural' => 'seo_bulk_editor_titles',
				'ajax' => true
			) );
		}
		...
		function display_tablenav( $which ) {
			...
			<div class="tablenav <?php echo esc_attr( $which ); ?>">
				...
				<form id="posts-filter" action="" method="get">
					<input type="hidden" name="page" value="seo_bulk_editor_titles" />
		...
		function get_views() {
			...
			$status_links['all'] = "<a href='admin.php?page=seo_bulk_editor_titles'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts' ), number_format_i18n( $total_posts ) ) . '</a>';
			...
			foreach( get_post_stati( array('show_in_admin_all_list' => true), 'objects' ) as $status ) {
				...
				$status_links[ $status_name ] = "<a href='admin.php?page=seo_bulk_editor_titles&amp;post_status=$status_name'$class>" . sprintf( translate_nooped_plural( $status->label_count, $total ), number_format_i18n( $total ) ) . '</a>';
			...
			$status_links[ 'trash' ] = "<a href='admin.php?page=seo_bulk_editor_titles&amp;post_status=trash'$class>" . sprintf( _nx( 'Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', $trashed_posts, 'posts' ), number_format_i18n( $trashed_posts ) ) . '</a>';
		...
		function get_columns() {
			return $columns = array(
				...
				'col_existing_aio_seo_title' => __('Existing All-in-One SEO Title'),
				'col_new_aio_seo_title' => __('New All-in-One SEO Title'),
		...
		function get_sortable_columns() {
			return $sortable = array(
				...
				'col_existing_aio_seo_title' => array( 'seo_title', false )
			);
		}
		...
		function prepare_items() {
			...
			$query = "SELECT ID, post_title, post_type, meta_value AS seo_title, post_status, post_modified FROM $wpdb->posts LEFT JOIN (SELECT * FROM $wpdb->postmeta WHERE meta_key = '_aioseop_title')a ON a.post_id = $wpdb->posts.ID WHERE post_status IN (%s)";
					...
					foreach( $columns as $column_name => $column_display_name ) {
						...
						switch( $column_name ) {
							...
							case 'col_post_status':
								...
								break;
							case 'col_existing_aio_seo_title':
								echo sprintf( '<td %2$s id="seo-bulk-editor-existing-title-%3$s">%1$s</td>', ( ($rec->seo_title ) ? $rec->seo_title : '' ), $attributes, $rec->ID );
								break;
							case 'col_new_aio_seo_title':
								$input = sprintf( '<input type="text" id="%1$s" name="%1$s" class="seo-bulk-editor-new-title" data-id="%2$s" />', 'seo-bulk-editor-new-title-' . $rec->ID, $rec->ID );
