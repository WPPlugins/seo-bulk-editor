<?php 
	...
	class SEO_Bulk_Editor_Description_List_Table extends WP_List_Table {

		function __construct() {
			parent::__construct( array( 
				'singular' => 'SEO_Bulk_Editor_description',
				'plural' => 'SEO_Bulk_Editor_descriptions',
				'ajax' => true
			) );
		}
		...
		function display_tablenav( $which ) {
			...
			<div class="tablenav <?php echo esc_attr( $which ); ?>">
				...
				<form id="posts-filter" action="" method="get">
					<input type="hidden" name="page" value="seo_SEO_Bulk_Editor_descriptions" />
		...
		function get_views() {
			...
			$status_links['all'] = "<a href='admin.php?page=seo_SEO_Bulk_Editor_descriptions'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts' ), number_format_i18n( $total_posts ) ) . '</a>';
			...
			foreach( get_post_stati( array('show_in_admin_all_list' => true), 'objects' ) as $status ) {
				...
				$status_links[ $status_name ] = "<a href='admin.php?page=seo_bulk-editor_descriptions&amp;post_status=$status_name'$class>" . sprintf( translate_nooped_plural( $status->label_count, $total ), number_format_i18n( $total ) ) . '</a>';
			...
			$status_links[ 'trash' ] = "<a href='admin.php?page=seo_bulk_editor_descriptions&amp;post_status=trash'$class>" . sprintf( _nx( 'Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', $trashed_posts, 'posts' ), number_format_i18n( $trashed_posts ) ) . '</a>';
		...
		function get_columns() {
			return $columns = array(
				...
				'col_existing_aio_seo_metadesc' => __('Existing All-in-One SEO Meta Description'),
				'col_new_aio_seo_metadesc' => __('New All-in-One SEO Meta Description'),
		...
		function get_sortable_columns() {
			return $sortable = array(
				...
				'col_existing_aio_seo_metadesc' => array( 'meta_desc', false )
			);
		}
		...
		function prepare_items() {
			...
			$query = "SELECT ID, post_title, post_type, meta_value AS meta_desc, post_status, post_modified FROM $wpdb->posts LEFT JOIN (SELECT * FROM $wpdb->postmeta WHERE meta_key = '_aioseop_description')a ON a.post_id = $wpdb->posts.ID WHERE post_status IN (%s)";
			...
			$per_page = $this->get_items_per_page( 'seo_bulk_editor_posts_per_page', 10 );
					...
					foreach( $columns as $column_name => $column_display_name ) {
						...
						switch( $column_name ) {
							...
							case 'col_post_status':
								...
								break;
							case 'col_existing_aio_seo_metadesc':
								echo sprintf( '<td %2$s id="seo-bulk-editor-existing-metadesc-%3$s">%1$s</td>', ( ($rec->meta_desc ) ? $rec->meta_desc : '' ), $attributes, $rec->ID );
								break;
							case 'col_new_aio_seo_metadesc':
								$input = sprintf( '<textarea id="%1$s" name="%1$s" class="seo-bulk-editor-new-metadesc" data-id="%2$s"></textarea>', 'seo-bulk-editor-new-metadesc-' . $rec->ID, $rec->ID );
								echo sprintf( '<td %2$s>%1$s</td>', $input , $attributes );
								break;
							case 'col_row_action':
								$actions = sprintf( '<a href="#" class="seo-bulk-editor-save" data-id="%1$s">Save</a> | <a href="#" class="seo-bulk-editor-save-all">Save All</a>', $rec->ID );
